...
import { datosStore } from './registerUser'

vi.mock('axios')

describe('store registro (registerUser.js)', () => {
  beforeEach(() => {
  ...
  it('guarda el usuario y limpia error/cargando cuando la llamada funciona', async () => {
    ...
    await store.registrarUsuario({ nombre: 'Juan', email: 'a@a.com', password: '12345678' })
  ...
  it('guarda error.response.data cuando el backend responde con error', async () => {
    const errorFake = { response: { data: 'El dni ya esta registrado' } }
    axios.post.mockRejectedValueOnce(errorFake)

    const store = datosStore()
    await expect(
      store.registrarUsuario({ dni: '12345678A' })
    ).rejects.toEqual(errorFake)

    expect(store.error).toBe('El dni ya esta registrado')
    expect(store.cargando).toBe(false)
  })
  ...
  it('llama a axios.post con la url del endpoint de registro', async () => {
    axios.post.mockResolvedValueOnce({ data: {} })
    const store = datosStore()

    await store.registrarUsuario({ nombre: 'Juan' })

    expect(axios.post).toHaveBeenCalledTimes(1)
    expect(axios.post.mock.calls[0][0]).toContain('/api/usuarios/registrar')
  })
})
